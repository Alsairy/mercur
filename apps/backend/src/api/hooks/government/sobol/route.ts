...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  const signature = (req.headers['x-sobol-signature'] as string) || ''
  const secret = process.env.SOBOL_API_KEY || ''
  if (!secret) {
    return res.status(500).json({ message: 'SOBOL_API_KEY not configured' })
  }
  ...
  const eventId =
    (req.headers['x-sobol-event-id'] as string) ||
